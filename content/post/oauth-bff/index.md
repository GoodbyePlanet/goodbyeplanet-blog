---
title: "Securing Your Web Apps: Spring Security OAuth 2.0 + BFF Pattern"
date: 2025-08-22
tags:  [security, spring, oauth2, bff-pattern, web-apps]
authors:
  - Nemanja
---
...
Spring OAuth 2.0 Resource Server and React as a frontend client.
...
front-end clients. One of the benefits of BFF and its architecture is that it makes easier to secure client applications.
...
For the demo purpose it has one controller for getting `/resource` that just returns text with the currently authenticated user.
...
It acts as a secure gateway between the frontend client and backend services, also it acts as a confidential client on the Authorization Server (`auth-server`)
...
with session data persisted in Redis for reliable state management.
...
#### Sequence Diagram
[Link to Mermaid sequence diagram](https://www.mermaidchart.com/play?utm_source=mermaid_js&utm_medium=editor_selection&utm_campaign=playground#pako:eNp9k09z2jAQxb_KDqfkQJvJ0YfMpAY7mcmkDKblksuOvBgNRnIkUZp2-t27srCR-RNfwPDT27e7T39HQpc0SkaW3nekBE0kVga3bwr4adA4KWSDysEPS-b812wKaGFFY1FL4vebOaFwt-dgvvRghY72-AE337LsAjSnUlrPhS8FWSu1gsJpQ-f0486tPew_tZF_0LUwmV-XnM6LIGz1zgjqsQBm0_HDQ75M4FlJJ7FmjsdhHewl10i13siDgXzJZGsvgXRNYtO57KSwdvCq4SfWsjz-B4eHj_P5bJq0LRoSYbDgNLzoSiqYYUVHvPXl-0vgUQgWu0h5wMt6pQQm0jY1zziQhTBEkQHP9JpT5bh2aqjk3XHb9i0ifSOZNDyEhdzS14JqditVBa-0Z1ndRA6uu0i1sj4Ypz7OvOTGb2lGZivbmdkjS6qMjXWF_L54n42vEBY1TELKyR5MvivVLZeHTr_FGlVFgIOj_lLASvu9bCi2MqztdkbB9zaH91_uYHECx1lpM3wgwO_ls2TEPRXk4GmxmI21qj_6KxFHMhpPn-NBfkMWr6U45wIYwtV2Cyujt0N_AS8SyLTZoymH8vHhwM-Lkxn1126CDnvNvt-OCW0HZvTvP3BmewA)
...
BFF gateway. Why is this important, and why not use an XHR request to call BFF backend?.
...
is the use of `ServerHttpSecurity` instead of `HttpSecurity`.
...
the user session is removed also on `auth-server` side. It also sets post logout redirect URL which is URL of `fe-client` React application.
...
It also configures how unauthenticated requests are handled, in this case for HTML request, it redirects to the
...
`.oauth2ResourceServer` configures this service as an OAuth 2.0 resource server. It also defines the default JWT configuration.
...
An important part of this service is [jwk-set-uri](https://github.com/GoodbyePlanet/spring-cg-bff/blob/45268092c70f4f92c30849403c7beb5b7808710d/secure-resource/src/main/resources/application.yml#L9)
which tells the Resource Server where to find the public keys for validating JWT tokens. This is a standard OAuth 2.0 endpoint
